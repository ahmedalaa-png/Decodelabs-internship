# 📊 DecodeLabs Data Analytics Internship
**Batch 2026 | Industrial Training Kit**

This repository contains my data analytics projects completed during the DecodeLabs internship program.

---

## 📁 Projects

| Project | Title | Status |
|---------|-------|--------|
| Project 1 | Data Cleaning & Preparation | ✅ Completed |
| Project 2 | Exploratory Data Analysis (EDA) | ✅ Completed |
| Project 3 | SQL Data Analysis | ✅ Completed |
---

## 🛠️ Tools & Technologies
- Python 3.14
- Pandas, NumPy
- Matplotlib, Seaborn
- Jupyter Notebook
- VS Code

---

## 👤 Author
**Ahmed Alaa**
GitHub: [@ahmedalaa-png](https://github.com/ahmedalaa-png)
